...
%===========================
ext_thumb.CE.F_max = 100;                 % F_max in [N] %source: Kistemaker2006, 
ext_thumb.CE.l_CEopt = 0.01;              % optimal length of CE in [m] %source: Kistemaker2006,

% serial elastic element (SEE)
% ============================
ext_thumb.SEE.l_SEE0        = 0.01;       % rest length of SEE in [m]

% load standard parameter set
ext_thumb = Library_mtu_simulink_mtu_standard_parameters(ext_thumb);

ext_thumb = Library_mtu_simulink_mtu_standard_parameters(ext_thumb);


%% load parameters for the activation dynamics
ext_thumb.ActDyn = Library_mtu_simulink_actdyn_standard_parameters;
...
ext_thumb.l_MTC_init = ext_thumb.CE.l_CEopt + ext_thumb.SEE.l_SEE0;  % [m] initial MTC length
ext_thumb.ActDyn.u_init  = 0;          % [] initial muscle activity 0...1
...
fhandle         = @(l_CE)init_muscle_force_equilib_with_Hatze_ActDyn(l_CE, ext_thumb.l_MTC_init, ext_thumb.ActDyn.u_init, ext_thumb, ext_thumb.ActDyn);
                         %init_muscle_force_equilib_with_Hatze_ActDyn(l_CE, l_MTC, u, MusParam, ActParam)
ext_thumb.CE.l_CE_init = fzero(fhandle, [0 ext_thumb.l_MTC_init]);
clear fhandle
